import numpy as np
import torch
from torch import tensor, norm, rfft, irfft, cat, mul, add, div
...
def _ncc_c(x, y):
    """
    # Variant of NCCc that operates with 1 dimensional X array and 1 dimensional y vector.
    # normalized cross correlation (with coefficient normalization)
...
def _kshape_pytorch(x, k, max_iterations):
    ...
    centroids = torch.zeros(k, len, device=x.device)
